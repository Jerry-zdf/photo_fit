...
int main(int argc, char *argv[])
{
    ...
    string output_file = "output/fit_" + input_file;
    ofstream output( output_file.c_str() );
    output << setprecision (15)<< expv.transpose()                     << endl
           << setprecision (15)<< ls_sol.head(expv.size()).transpose() << endl
           << setprecision (15)<< ls_sol.tail(expv.size()).transpose() << endl;
    output.close();
